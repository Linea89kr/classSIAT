...
public class HomeController {
    ...
    public String home(){
        // webapp 안의 /WEB-INF/views/index.jsp 를 사용자에게 전달
        return "index";
    }

    // URL 주소
    /* - 사용자가 서버에 요청하기 위해 호출하는 주소
    *  - URL 주소는 여러 의미를 가지고 있는 값들로 구성되며 Spring Boot Web 에서는 해당 URL을 처리하기 위한 각종
    *    기능을 제공하는 어노테이션들을 제공한다.
    * */

    // @Controller
    /* - 클라이언트에서 요청한 데이터를 처리하거나 사용자 쪽으로 처리한 응답을 제공할 때 사용하는 클래스 어노테이션
    *  - 이 어노테이션이 선언된 클래스는 컨트롤러 클래스로써 사용자의 요청과 응답을 처리하는 클래스로 정의하게 된다.
    *  - @Controller에는 실제 URL 경로와 사용자들이 요청과 함께 보낸 값을 처리할 수 있는 여러가지 기능을 제공하도록
    *    메서드에 기능을 부여한다.
    * */
    ...
    *  - 하나 차이가 있다면 @RequestMapping은 메소드 뿐만이 아닌 클래스에도 사용이 가능하다.
    *  - @RequestMApping 에는 두가지의 값을 지정할 수 있으며 해당 element는 다음과 같다.
    *    => value : 컨텍스트 패스 외의 하위 주소 경로를 지정한다.
    *    => method : 클라이언트에서 보내는 특정 신호에만 값을 받을 수 있도록 한다.
    * */

    // @Controller method
    /* - @Controller 내에 속한 method는 return 타입을 보통 string으로 내보내며 해당 string 은 jsp 기준 prefix와 suffix
    *    를 앞 뒤로 붙여 참조하고자 하는 웹 페이지를 가리키게 된다.
    *  - 해당 웹 페이지를 가리켜 사용자가 후에 보고자 하는 페이지의 데이터를 보내주게 되며 이 페이지를 사용자는 pulling 방식을 통해
    *    제공 받게 된다.
